...
const timeScaleOptionsAirflow = {
    responsive: true,
    scales: {
        x: { type: 'time', time: { unit: 'minute' }, ticks: {
            display: false, // 🔒 Hide timestamps (tick labels)
          } },
      y: {
        ticks: {
          stepSize: 1,
  ...
  <Link to="/sessions">
    ...
    <button onClick={handlePrint}>Print Graphs</button>
      ...
      <div className="chart-grid" style={containerStyle}>
        ...
        <div className="chart-row" style={{ backgroundColor: 'white', padding: '10px' }}>
  <Line
    data={heartRateTrendData}
    options={{ ...oxygenTrendOptions, maintainAspectRatio: false }}
  />
</div>
        <div className="chart-row">
        ...
        <div className="chart-row">
          <Line data={airflowChartData} options={{ ...timeScaleOptionsAirflow, maintainAspectRatio: false }} />
        </div>
        <div className="chart-row">
